...
func TestRegisterWithHostInfo(t *testing.T) {
	env := newTestEnv(t)
	resp, body := env.do("POST", "/v1/devices",
		`{"name":"edge-1","kind":"orangepi-edge","version":"0.1.0","hostname":"opi4","arch":"aarch64","os":"Linux 6.1"}`)
	if resp.StatusCode != http.StatusCreated {
		t.Fatalf("register: %d", resp.StatusCode)
	}
	if body["hostname"] != "opi4" || body["arch"] != "aarch64" || body["os"] != "Linux 6.1" {
		t.Errorf("host info missing: %v", body)
	}
	devID, _ := body["id"].(string)

	// GET 验证持久化
	resp, body = env.do("GET", "/v1/devices/"+devID, "")
	dev := body["device"].(map[string]any)
	if dev["hostname"] != "opi4" {
		t.Errorf("GET hostname: %v", dev["hostname"])
	}
}
...
func TestHeartbeatWithMetricsAndSession(t *testing.T) {
	env := newTestEnv(t)
	resp, body := env.do("POST", "/v1/devices",
		`{"name":"edge-1","kind":"orangepi-edge","version":"0.1.0","hostname":"opi4","heartbeat_interval_ms":5000}`)
	if resp.StatusCode != http.StatusCreated {
		t.Fatalf("register: %d", resp.StatusCode)
	}
	devID, _ := body["id"].(string)

	// 发心跳(带 metrics + session_id)
	resp, _ = env.do("POST", "/v1/devices/"+devID+"/heartbeats",
		`{"seq":1,"session_id":"sess-abc","metrics":{"cpu_percent":12.5,"memory_percent":34.0,"can_available":false,"runtime_state":"idle"}}`)
	if resp.StatusCode != http.StatusAccepted {
		t.Fatalf("heartbeat with metrics: %d", resp.StatusCode)
	}

	// 查询验证
	resp, body = env.do("GET", "/v1/devices/"+devID, "")
	hb := body["last_heartbeat"].(map[string]any)
	if hb["session_id"] != "sess-abc" {
		t.Errorf("session_id: %v", hb["session_id"])
	}
	metrics := hb["metrics"].(map[string]any)
	if metrics["cpu_percent"] != 12.5 || metrics["can_available"] != false || metrics["runtime_state"] != "idle" {
		t.Errorf("metrics: %v", metrics)
	}

	// 重启(新 session_id,温度 unavailable)
	resp, _ = env.do("POST", "/v1/devices/"+devID+"/heartbeats",
		`{"seq":2,"session_id":"sess-def","metrics":{"cpu_percent":8.0,"memory_percent":30.0,"runtime_state":"idle"}}`)
	if resp.StatusCode != http.StatusAccepted {
		t.Fatalf("heartbeat new session: %d", resp.StatusCode)
	}
	resp, body = env.do("GET", "/v1/devices/"+devID, "")
	hb2 := body["last_heartbeat"].(map[string]any)
	if hb2["session_id"] != "sess-def" {
		t.Errorf("new session_id: %v", hb2["session_id"])
	}
	m2 := hb2["metrics"].(map[string]any)
	// temperature 未提供→应不存在
	if _, ok := m2["temperature_celsius"]; ok {
		t.Errorf("temperature should be absent when not provided, got %v", m2)
	}
}
