...
func (s *Store) migrate() error {
	...
	// 分条执行:CREATE TABLE 无问题;ALTER TABLE 在新库上会报"duplicate column"
	// 存库上为迁移。
	parts := strings.Split(string(schema), ";")
	for _, p := range parts {
		...
		if _, err := s.db.Exec(p); err != nil {
			...
			return fmt.Errorf("migrate %q: %w", p[:min(60, len(p))], err)
		}
	}
	return nil
}
...
func (s *Store) CreateDevice(ctx context.Context, d *domain.Device) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO devices (id, name, kind, version, hostname, arch, os, heartbeat_interval_ms, first_seen_ms, last_seen_ms)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0)`,
		d.ID, d.Name, d.Kind, d.Version, d.Hostname, d.Arch, d.OS, d.HeartbeatIntervalMs, d.FirstSeenMs)
...
func (s *Store) GetDevice(ctx context.Context, id string) (*domain.Device, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, name, kind, version, hostname, arch, os,
		 heartbeat_interval_ms, first_seen_ms, last_seen_ms
		 FROM devices WHERE id = ?`, id)
	var d domain.Device
	if err := row.Scan(&d.ID, &d.Name, &d.Kind, &d.Version,
		&d.Hostname, &d.Arch, &d.OS,
...
//   - 设备必须存在;
//   - 成功后同步设备 last_seen_ms(hostname/arch/os 若心跳携带则更新)。
//   - metrics 序列化为 JSON 存储。
//
...
func (s *Store) AddHeartbeat(ctx context.Context, h domain.Heartbeat) error {
	...
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO heartbeats (device_id, seq, ts_ms, session_id, metrics)
		 VALUES (?, ?, ?, ?, ?)`,
		h.DeviceID, h.Seq, h.TsMs, h.SessionID, metricsJSON); err != nil {
		return err
	}
...
// LastHeartbeat 返回设备最近一次心跳(含 session_id 与解析后的 metrics)。
func (s *Store) LastHeartbeat(ctx context.Context, deviceID string) (*domain.Heartbeat, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT device_id, seq, ts_ms, session_id, metrics FROM heartbeats
		 WHERE device_id = ? ORDER BY seq DESC LIMIT 1`, deviceID)
	var h domain.Heartbeat
	var metricsJSON string
	if err := row.Scan(&h.DeviceID, &h.Seq, &h.TsMs, &h.SessionID, &metricsJSON); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, ErrNotFound
		}
		return nil, err
